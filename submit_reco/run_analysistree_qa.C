...
void run_analysistree_qa(std::string filelist="list", bool is_single_file = false);
...
void TofHitsQA(QA::Task& task)
{
  ...
  task.AddH1({"TOF hit z-position (cm)", {tof_hits, "z"}, {QA::gNbins, 700, 900}});
...
void SimEventHeaderQA(QA::Task& task)
{
  ...
  task.AddH1({"z_{vertex}^{MC} (cm)", {sim_event_header, "vtx_z"}, {QA::gNbins, -0.5, 0.5}});
  task.AddH1({"z_{vertex}^{MC} (cm)", {sim_event_header, "vtx_z"}, {QA::gNbins, -40.5, -39.5}});
  task.AddH1({"z_{vertex}^{MC} (cm)", {sim_event_header, "vtx_z"}, {QA::gNbins, -44.5, -43.5}});
...
void RecEventHeaderQA(QA::Task& task)
{
  ...
  Variable eventLength ("eventLength", {{rec_event_header, "start_time"}, {rec_event_header, "end_time"}}, []( std::vector<double>& var ) { return var.at(1)-var.at(0); });
  Variable deltaTRecoSim ("deltaTRecoSim", {{rec_event_header, "start_time"}, {sim_event_header, "start_time"}}, []( std::vector<double>& var ) { return var.at(0)-var.at(1); });
  task.AddH1({"Reco event length (ns)", eventLength, {1000, 0, 5000}});
  task.AddH1({"T_{START}^{reco}-T_{START}^{sim} (ns)", deltaTRecoSim, {1000, 0, 10000}});
  ...
  task.AddH1({"z_{vertex} (cm)", {rec_event_header, "vtx_z"}, {QA::gNbins, -0.5, 0.5}});
  task.AddH1({"z_{vertex} (cm)", {rec_event_header, "vtx_z"}, {QA::gNbins, -40.5, -39.5}});
  task.AddH1({"z_{vertex} (cm)", {rec_event_header, "vtx_z"}, {QA::gNbins, -44.5, -43.5}});
  ...
  task.AddH2({"z_{vertex} (cm)", {rec_event_header, "vtx_z"}, {QA::gNbins, -0.5, 0.5}},
             {"z_{vertex} (cm)", {sim_event_header, "vtx_z"}, {QA::gNbins, -0.5, 0.5}});
  task.AddH2({"z_{vertex} (cm)", {rec_event_header, "vtx_z"}, {QA::gNbins, -40.5, -39.5}},
             {"z_{vertex} (cm)", {sim_event_header, "vtx_z"}, {QA::gNbins, -40.5, -39.5}});
  task.AddH2({"z_{vertex} (cm)", {rec_event_header, "vtx_z"}, {QA::gNbins, -44.5, -43.5}},
             {"z_{vertex} (cm)", {sim_event_header, "vtx_z"}, {QA::gNbins, -44.5, -43.5}});
}
